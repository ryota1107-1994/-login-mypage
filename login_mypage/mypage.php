<?php
    ...
    if (empty($_SESSION['id'])) {
        ...
        // ログイン時のメールアドレスとパスワードからユーザ情報を取り出す
        ...
        //fetcht・while文でデータを取得し、sessionに代入
        // データベースから取り出したユーザ情報をセッションにセットする
        ...
        // データ取得できずに(emptyを使用して判定) sessionがなければ、リダイレクト(エラー画面へ)
        // セッションがセットされていなければエラーページに遷移する
        if(empty($_SESSION['id'])){
    ...
    // ユーザ情報が取り出せた　かつ　「ログイン情報を保持する」にチェックがあれば、クッキーに値をセット
    // 「ログイン情報を保持する」にチェックがなければクッキーを削除する
